// App.tsx – Slowpitch Fantasy MVP
// - Upload Monday + Friday GameChanger TOTALS CSVs
// - Parse ONLY batting stats (cut off at TB) to avoid duplicate column collisions
// - Compute points + points/PA using locked scoring (ROE = +1)
// - Offline draft: assign players to Martiny or Stufflebean + pick MON sub + FRI sub
// - Prevent duplicates across both teams + subs
...
import type { LeagueTag, Team, PlayerTotals, OrderedTeam, Night, TeamState, } from "./types";
...
function deepFreezeLineup(team: Team) {
  return JSON.stringify({
    starters: team.starters,
    bench: team.bench,
    activeByNight: team.activeByNight,
  });
}
...
function poolHasRealStats(pool: PlayerTotals[]) {
  return pool.some(p => p.PA > 0 || p.points !== 0);
}
...
export default function App() {
  ...
  useEffect(() => {
    getCurrentSeason().then(season => {
      ...
      setState(prev => {
        // ⛔ do NOT stomp week if already correct
        if (prev.week === season.currentWeek) return prev;
  ...
  function normalizeDraftRoster(team: Team) {
    // draft-only logic
  }

  function normalizeWeeklyRoster(team: Team) {
    // 🚫 ABSOLUTELY NO weekly normalization
    return;
  }
  ...
  function applyWeeklyLineups(
    ...
    // 🧠 Freeze draft roster ONCE — this is the source of truth
    const draftRosters: string[][] = teams.map(team =>
      Array.from(new Set([...team.starters, ...team.bench]))
    );
  ...
  async function saveWeeklyLineupFromTeam(
    ...
    const url = `http://localhost:5000/api/lineups/${seasonId}/${week}`;
    ...
    const res = await authFetch(url, {
      ...
      body: JSON.stringify({
        teamId: dbTeamId,
        night,               // ✅ include this if your API needs it
  ...
  useEffect(() => {
    let loadedSeasonId: number;

    getCurrentSeason()
      .then(season => {
        loadedSeasonId = season.seasonId;
        setSeasonId(season.seasonId);
        return Promise.all([
      ...
      .catch(err => {
        console.warn("⚠️ Failed loading season data", err);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    authFetch(`http://localhost:5000/api/scores/season/${seasonId}`)
      .then(async res => {
        if (!res.ok) {
  ...
  function applyLeaguesFromUploads(
    pool: PlayerTotals[],
    uploads: { MON: PlayerTotals[]; FRI: PlayerTotals[] }
  ): PlayerTotals[] {
    // Clone pool and reset leagues
    const byKey = new Map(
      pool.map(p => [p.key, { ...p, leagues: [] as LeagueTag[] }])
    );

    (["MON", "FRI"] as LeagueTag[]).forEach(night => {
      uploads[night].forEach(p => {
        const target = byKey.get(p.key);
        if (!target) return;

        if (!target.leagues.includes(night)) {
          target.leagues.push(night);
        }
      });
    });
  ...
  await fetch("http://localhost:5000/api/players/sync", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const pendingSaveRef = useRef<{ teamIdx: 0 | 1; night: "MON" | "FRI" } | null>(null);
  ...
  async function startNewWeek() {
    ...
    await authFetch(
      `http://localhost:5000/api/scores/${seasonId}/${state.week}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify([
    ...
    await authFetch(
      `http://localhost:5000/api/seasons/${seasonId}/advance-week`,
      { method: "POST" }
    );
  ...
  function lockPlayersForNight(
    team: Team,
    night: "MON" | "FRI",
    uploads: AppState["uploads"]
  ) {
    const playedKeys = new Set(uploads[night].map(p => p.key));
    const captainKey = getCaptainKey(team);

    const newlyLocked = team.activeByNight[night].filter(
      k => playedKeys.has(k) && k !== captainKey
    );

    team.lockedByNight[night] = Array.from(
      new Set([...team.lockedByNight[night], ...newlyLocked])
    );
